<?php
	...
	{
		global $conn;
		$sel = "select mname from cz_menu where mid='".$mid."'";
	...
	{
		global $conn;
		$sel = "select rid from cz_role where uname='".$uname."'";
		$qry = mysqli_query($conn,$sel);
		if(mysqli_num_rows($qry)>0)
		{
			return "Yes";
	...
	{
		global $conn;
		$uname = $_SESSION['cz_utype'];
		//$sel = "select rstatus from cz_role where uname='".$uname."' and rname='".$mtitle."'";
		$sel = mysqli_query($conn,"select rstatus from cz_role where uname='".$uname."' and rname='".$mtitle."'")or die(mysqli_error($conn));
	...
	function get_exname_photo($uid)
	{
		...
		if($uid == "")
		{
			...
			$sel = mysqli_query($conn,"select photo from cz_login where email='$uid'");
			if(mysqli_num_rows($sel)>0)
			{
				$fet = mysqli_fetch_array($sel);
				return $fet['photo'];
	...
	function get_exname($uid)
	{
		...
		if($uid == "")
		{
			...
			$sel = mysqli_query($conn,"select fname,lname from cz_login where id='$uid'");
			if(mysqli_num_rows($sel)>0)
			{
				$fet = mysqli_fetch_array($sel);
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='cname'");
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='clogo'");
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='caddress'");
		$row = mysqli_fetch_array($result);
		return $row['svalue'];
	}
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='cemailid'");
		$row = mysqli_fetch_array($result);
		return $row['svalue'];
	}
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='ADMIN_FEE'");
		$row = mysqli_fetch_array($result);
		return $row['svalue'];
	}
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='GST'");
		$row = mysqli_fetch_array($result);
		return $row['svalue'];
	}
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='cmobile'");
		$row = mysqli_fetch_array($result);
		return $row['svalue'];
	}
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_settings where sname='cphone'");
		$row = mysqli_fetch_array($result);
		return $row['svalue'];
	}
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select fname,lname from cz_login where id='".$_SESSION['cz_admin_login_id']."'");
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select photo from cz_login where id='".$_SESSION['cz_admin_login_id']."'");
		$num = mysqli_num_rows($result);
		if($num > 0)
		{
			$row = mysqli_fetch_array($result);
	...
	function UploadFile($file,$folder_name,$file_name="",$index="-1")
	{
		...
		if($index=="-1")
		{
			if($file_name=="")
			{
				$path_parts = pathinfo($_FILES[$file]["name"]);
				$extension = $path_parts['extension'];
				$file_name=base64_encode(uniqid()."_".date('H:i:s')).".".$extension;	
			}
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_customers where cid='".$id."'");
	...
	{
		global $conn;
		$result = mysqli_query($conn,"select * from cz_driver where cid='".$id."'");
		while($row = mysqli_fetch_array($result))
		{
			$token[]=$row['reg_token_ids'];
		}
	...
	function SendNotification($title,$body,$receipient_token)
	{
		...
		$post_data_arr['data']['title']=$title;
		$post_data_arr['data']['body']=$body;
	...
	function AddNotification($title,$description,$type,$type_id,$id_recepient,$is_read="1",$id_notification="0")
	{
		...
		{
			$add_remarks=mysqli_query($conn,"UPDATE cz_notification_details SET title='$title',id_recepient='$id_recepient',description='$description',type='$type',type_id='$type_id',is_read='$is_read',date='$date_added' WHERE id_notification='$id_notification'" )or die(mysqli_error($conn));	
			return "1";
		}else
		{
			$add_remarks=mysqli_query($conn,"INSERT INTO cz_notification_details (id_notification,title,id_recepient,description,type,type_id,is_read,date) values(NULL,'$title','$id_recepient','$description','$type',$type_id,'$is_read','$date_added')")or die(mysqli_error($conn));
			if($add_remarks)
			{
	...
	{
		global $conn;
		$sel_crime_details=mysqli_query($conn,"SELECT * from cz_crime_details WHERE id='$id_crime' ");
		if(mysqli_num_rows($sel_crime_details)>0)
		{
			...
			if($fet_crime_details['is_requested_update']=="0" && $fet_crime_details['is_open_update']!="0")
			{
				...
				{
					mysqli_query($conn,"UPDATE cz_crime_details SET is_open_update='0',is_requested_update='0',update_request_status='0',update_request_date='' WHERE id='$id_crime' ");
				}
			}else
			{
	...
	{
		global $conn;
		$res_check=mysqli_query($conn,"SELECT * FROM cz_user_wallet where id_user='$id_user' AND utype='$utype' ")or die(mysqli_error($conn));
	...
	function CreateWalletIfNotExist($id_user,$utype="0")
	{
		...
		{
			$res_check=mysqli_query($conn,"INSERT INTO cz_user_wallet (id_user,date_updated,utype) VALUES('".$id_user."','".date('Y-m-d H:i:s')."','$utype')")or die(mysqli_error($conn));
			if($res_check)
			{
	...
	function AddWalletBalance($id_user,$add_amount,$utype="0")
	{
		...
		$res_check=mysqli_query($conn,"UPDATE cz_user_wallet SET balance='".$balance."',date_updated='".date('Y-m-d H:i:s')."' WHERE id_user='".$id_user."' AND utype='".$utype."' ")or die(mysqli_error($conn));
		if($res_check)
		{
			$add_summary=mysqli_query($conn,"INSERT INTO cz_user_wallet_summary (id_user,utype,amount,Transaction_type) VALUES('$id_user','$utype','$add_amount','Credit') ");
			return "1";
		}else
		{
			return "0";
	...
	function SubtractWalletBalance($id_user,$subtract_amount,$utype="0")
	{
		...
		{
			return "1";
			$add_summary=mysqli_query($conn,"INSERT INTO cz_user_wallet_summary (id_user,utype,amount,Transaction_type) VALUES('$id_user','$utype','$subtract_amount','Debit') ");
		}else
		{
			return "0";
		}
	...
	function UpdateWalletBalance($id_user,$balance,$utype="0")
	{
		global $conn;
		CreateWalletIfNotExist($id_user,$utype);
		$res_check=mysqli_query($conn,"UPDATE cz_user_wallet SET balance='".$balance."',date_updated='".date('Y-m-d H:i:s')."' WHERE id_user='".$id_user."' AND utype='".$utype."' ");
	...
	function CreateTransaction($transaction_arr)
	{
		...
		$Insert_query=CreateInsertQuery($transaction_arr,"cz_transaction");	
		// $balance=$wallet_balance+$add_amount;
...
/*
	function CheckIsAvailCouponCode($id_user,$coupon_code,$date="")	
	{
		...
		$coupon_code_details=GetConditionalDetailsFromTable($table_name="cz_coupon_details",$condition,$is_multiple="0","0");
	...
	function CheckIsCouponCodeAvailUser($id_user,$coupon_code,$amount,$date="")	
	{
		...
		{
			$sel_check_coupon_used=mysqli_query($conn," SELECT *  from cz_coupon_details where user_id='".$user_id."' ");
		...
		if($is_avail=="1")
		{
			...
			{
				$total_used_coupon=GetTotalAvailFromTable("cz_coupon_apply_details",$is_stat_chk="0"," coupon_code='".$coupon_code."' ");
	...
	function ApplyCoupon($id_user,$coupon_code,$transaction_amount,$date="",$id_ride="",$id_transaction="")	
	{
		...
		if($is_valid_coupon=="1")
		{
			...
			$apply_coupon_code=mysqli_query($conn," INSERT INTO cz_coupon_apply_details (id_coupon,coupon_code,user_id,transaction_amount,discount_amount,id_transaction,id_ride,date_added) VALUES('".$id_coupon."','".$coupon_code."','".$id_user."','".$transaction_amount."','".$discount_amount."','".$id_transaction."','".$id_ride."','".$date."')");

			// apply to ride
			mysqli_query($conn,"UPDATE cz_ride_offers_details set coupon_discount='".$discount_amount."',coupon_code='".$coupon_code."' WHERE id='$id_ride' ");
	...
	function get_admin_settings($field="")
	{
		global $conn;
		$return_val="";
		$result = mysqli_query($conn,"select * from cz_settings where sname='".$field."'");
	...
	function ProcessReferralBonus($refer_by,$referral_code,$utype="0")
	{
		...
		if($utype=="0")
		{
			$table_name="cz_customers";
		}else
		{
			$table_name="cz_driver";
		}
		...
		if(mysqli_num_rows($res_check)>0)
		{
			...
			// print_r($fet_user);
			mysqli_query($conn,"INSERT INTO cz_user_wallet (id_user,date_updated,utype) VALUES('".$refer_by_user."','".date('Y-m-d H:i:s')."','$utype')")or die(mysqli_error($conn));
	...
	function SendCrimeAlertNotification($crime_id,$logged_in_user)
	{
		...
		$crime_details= GetConditionalDetailsFromTable("cz_crime_details"," id='$crime_id' ","0","0");
		if($crime_details!="")
		{
			...
			$crime_type_name=GetSingleFieldDataFromTable("cz_crime_type","name"," id='".$crime_type."' ","0");
			...
			$alert_range_details=GetConditionalDetailsFromTable("cz_user_alert"," status='1' ","1","0");
			if($alert_range_details!="")
			{
				...
				{ 
					...
					{
						$alert_user_device_token=GetSingleFieldDataFromTable("cz_users","reg_token_ids"," cid='".$alert_user_id."' ","0");
